...
<?php } else { ?>
	...
	<ul class="subsubsub">
		<li>			
			...
			<select id="filter" name="filter">
				...
				<option value="starting-from-order-id" <?php selected($filter, "starting-from-order-id");?>><?php _e('Starting From Order ID X', 'pmpro');?></option>
			</select>
	...
	<input id='starting-order-id' name="starting-order-id" type="text" size="4" value="<?php echo $starting_order_id ?>" />
		...
		function pmpro_ShowMonthOrYear()
		{
			...
			else if(filter == 'within-a-status')
			{
				...
				jQuery('#filterby').show();
	                ...
	                else if(filter == 'starting-from-order-id')
			{
				jQuery('#start-month').hide();
				jQuery('#start-day').hide();
				jQuery('#start-year').hide();
